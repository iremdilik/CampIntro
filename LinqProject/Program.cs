    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var result = from p in products
                         ...
                         on p.CategoryId equals c.CategorId
                         where p.UnitPrice>5000
                         orderby p.UnitPrice descending
                         select new ProductDto {ProductId = p.ProductId, CatogoryName = c.CategoryName, ProductName=p.ProductName, UnitPrice = p.UnitPrice};

            foreach (var productDto in result)
            {
                Console.WriteLine("{0}-{1}", productDto.ProductName, productDto.CatogoryName);
            }
        }
